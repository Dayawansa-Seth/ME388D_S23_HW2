...
# Lists to store k_eff and boron values
k_eff_list = []
boron_list = []
energy_list = []
critical_flux_list = []

# Function to extract k-eff and boron values from .out files
def extract_values(filename):
    ...
    with open(filename, 'r') as file:
        ...
        for line in lines:
            ...
            # Extract boron value (on line 39, starting at column 27)
            if "atomboh3" in line:
                boron_match = re.search(r"atomboh3\s*[\d\.]+\s*([\d\.]+)", line)  # Extract the number before space
                if boron_match:
                    boron = float(boron_match.group(1))  # Convert the extracted string to float
    ...
    return k_eff, round(boron,2), critical_flux, critical_energy
...
for root, dirs, files in os.walk('.'):
    for file in files:
        if file.endswith('.out'):
            ...
            k_eff, boron, critical_flux, critical_energy   = extract_values(filepath)
            
            if k_eff is not None and boron is not None:
                k_eff_list.append(k_eff)
                boron_list.append(boron)
...
# Plotting the data
plt.plot(boron_list, k_eff_list, 'bo-')
plt.xlabel("boron concentration")
plt.ylabel("k-eff")
plt.title("boron concentration vs k-eff")
plt.grid(True)
plt.show()
...
for i in range(len(critical_flux_list)):
    if i%1 == 0:
        plt.semilogx(energy_list[i], critical_flux_list[i], label=f"{boron_list[i]} g/cm^3")

# Add titles and labels
plt.title('Critical Flux Values Over a Variety of Boron Concentrations')
